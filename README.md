...
### 1. 매장 코드 로그인 시스템 (`app/page.tsx`)

**핵심 가치**: 간편하고 안전한 매장 인증 시스템으로 점주님의 개인화된 대시보드를 제공합니다.

**주요 기능**:

- **스마트 입력 검증**
  - 실시간 입력 필터링: 숫자만 입력 가능, 자동으로 5자리로 제한
  - 즉시 피드백: 입력 형식 오류 시 즉시 에러 메시지 표시
  - 사용자 친화적 UI: 포커스 시 초록색 테두리, 에러 시 빨간색 강조

- **강화된 보안 검증**
  - Supabase `매장마스터` 테이블에서 실시간 매장 존재 여부 확인
  - 다중 형식 지원: 문자열/숫자 형식 모두 자동 변환하여 검색
  - 존재하지 않는 매장 코드 차단으로 무단 접근 방지

- **세션 관리**
  - `sessionStorage`를 활용한 클라이언트 사이드 세션 관리
  - 브라우저 탭 종료 시 자동 로그아웃으로 보안 강화
  - 모든 페이지에서 매장 코드 자동 전달

- **사용자 경험**
  - 로딩 상태 표시: "로그인 중..." 버튼으로 진행 상황 안내
  - 부드러운 전환: 로그인 성공 시 개요 페이지로 자연스러운 이동
  - 반응형 디자인: 모바일/데스크톱 최적화된 UI

**기술적 특징**:
- Client Component로 실시간 상호작용 지원
- Next.js Router를 활용한 프로그래밍 방식 네비게이션
- 에러 핸들링 및 사용자 피드백 최적화

---

### 2. 서비스 개요 대시보드 (`app/overview/page.tsx`)

**핵심 가치**: 한눈에 보는 매장 현황과 핵심 인사이트를 제공하여 빠른 의사결정을 지원합니다.

**주요 기능**:

- **추천 상품 프리뷰**
  - 상위 15개 추천 상품 자동 캐러셀 (3초 간격)
  - 상품 이미지, 이름, 추천 점수, 추천 이유 표시
  - 클릭 시 추천 상품 페이지로 이동하여 상세 정보 확인 가능
  - 유사 매장에서 잘 팔리는 상품을 우선적으로 표시

- **부진 상품 프리뷰**
  - 상위 15개 부진 상품 자동 캐러셀 (3초 간격)
  - 판매 부진 이유 및 개선 방안 제시
  - 클릭 시 부진 상품 페이지로 이동하여 상세 분석 확인
  - 재고 관리 및 발주 최적화를 위한 인사이트 제공

- **서비스 소개 및 알고리즘 설명**
  - **유사 매장 선정 기준 모달**: 유사도 계산 방법 및 기준 설명
  - **Rolling Window 알고리즘 모달**: 시계열 데이터 분석 방법론 설명
  - **동적 상권 포착 모달**: 상권 변화 감지 및 대응 전략 설명
  - 투명한 AI 의사결정 과정으로 점주님의 신뢰 확보

- **반응형 대시보드**
  - 모바일: 간결한 카드 형태, 하단 네비게이션
  - 데스크톱: 풍부한 정보 표시, 상단 네비게이션
  - PWA 앱 환경 자동 감지 및 최적화
...
**데이터 소스**:
- Supabase `{매장명}_추천상품` 테이블
- Supabase `{매장명}_부진재고` 테이블
- `상품마스터` 테이블 (상품 이미지 조회)

---

### 3. 추천/부진 상품 상세 조회 (`app/recommendations/page.tsx`)

**핵심 가치**: 데이터 기반 상품 발주 의사결정을 지원하는 핵심 기능입니다.

**주요 기능**:

#### 추천 상품 탭 (`?tab=recommended`)

- **지능형 상품 추천**
  - 유사 매장에서 실제로 잘 팔리는 상품 우선 추천
  - RFM 분석 기반 추천 점수 산정
    - **Recency**: 최근 구매 빈도
    - **Frequency**: 구매 빈도
    - **Monetary**: 구매 금액
  - 예측 점수 (`pred_score`, `pred_rfm`)를 통한 판매 가능성 예측

- **카테고리별 필터링**
  - 대분류 필터: 과자, 냉장, 맥주, 면, 미반, 빵, 음료
  - 중분류 다중 선택 필터
  - 실시간 필터링으로 원하는 카테고리만 빠르게 확인
...
- **상품 상세 모달**
  - 정보 탭: 상품 기본 정보, 가격, 카테고리
  - 이유 탭: 추천/부진 이유 상세 설명
  - 점수 가이드: 추천 점수 산정 기준 설명
...
- Supabase `{매장명}_추천상품` 테이블
- Supabase `{매장명}_부진재고` 테이블
- `상품마스터` 테이블 (상품 이미지 및 메타데이터)

---

### 4. 유사 매장 리포팅 및 분석 (`app/similar-stores/page.tsx`)

**핵심 가치**: 유사 매장과의 비교 분석을 통해 경쟁력 강화 및 발주 최적화 전략을 수립합니다.
...
- **유사도 기반 매장 추천**
  - 유사도 점수 (0-100) 순으로 정렬
  - 유사도 근거 설명 (주요 2-3개 항목)
  - 매장명, 주소, 전화번호, 영업시간, 매장면적 정보 제공

- **인터랙티브 카카오 지도**
  - 현재 매장 및 유사 매장 위치 시각화
  - 커스텀 마커: 매장명과 로고 표시
  - 매장 클릭 시 지도에서 해당 위치로 자동 이동
  - 선택된 매장 강조 표시 (크기 증가, z-index 조정)
  - 주소 검색 및 좌표 변환 자동 처리
...
- **월별 데이터 선택**
  - 드롭다운으로 분석 기준 월 선택
  - 최신 데이터부터 과거 데이터까지 조회 가능
  - 선택한 월에 맞춰 모든 분석 데이터 자동 업데이트

#### 판매 패턴 비교 분석

- **카테고리별 판매 패턴 비교**
  - Radar Chart로 7개 카테고리 비교 (과자, 냉장, 맥주, 면, 미반, 빵, 음료)
...
- **시간대별 판매 패턴 분석**
  - 주중/주말 탭 전환
  - Line Chart로 시간대별 트렌드 비교
  - 피크 타임 및 저조 시간대 식별
  - AI 기반 인사이트 제공

- **주중/주말 패턴 분석**
  - Pie Chart로 주중/주말 비율 시각화
  - 비율 차이 분석
  - 매장 특성 파악 (주말 중심형, 주중 중심형, 균형형)
  - AI 기반 분석 텍스트 제공
...
**AI 기반 분석**:
- OpenAI GPT-4o-mini를 활용한 자연어 분석 텍스트 생성
- 전문적이고 이해하기 쉬운 설명
- 데이터 기반 객관적 인사이트 제공

**데이터 소스**:
- Supabase `매장마스터` 테이블 (유사 매장 목록)
- 월별 패턴 데이터
- 카테고리별 판매 데이터
- 시간대별 판매 데이터
...
### 5. AI 기반 분석 텍스트 생성 (`app/api/analyze-similarity/route.ts`)

**핵심 가치**: 복잡한 데이터를 점주님이 이해하기 쉬운 자연어로 변환하여 의사결정을 지원합니다.
...
- **3가지 분석 타입 지원**
  ...
  1. **판매패턴 분석**
     - 카테고리별 판매 비율 비교
     - 가장 유사한 카테고리 및 차이가 큰 카테고리 식별
     - 유사도 점수 기반 분석

  2. **시간대패턴 분석**
     - 주중/주말 시간대별 판매 패턴 비교
     - 가장 유사한 시간대 및 피크 타임 식별
     - 시간대별 판매 집중도 분석

  3. **주중주말패턴 분석**
     - 주중과 주말의 판매 비율 비교
     - 매장 특성 분류 (주말 중심형, 주중 중심형, 균형형)
     - 비율 차이 분석

- **지능형 프롬프트 생성**
  - 분석 타입에 맞는 최적화된 프롬프트 자동 생성
  - 핵심 데이터만 추출하여 전달
  - 전문적이면서도 이해하기 쉬운 설명 요청

- **OpenAI GPT-4o-mini 활용**
  - 비용 효율적인 모델 사용
  - 빠른 응답 시간
  - 전문적인 어조로 일관된 분석 텍스트 생성

- **응답 최적화**
  - 최대 2줄 이내로 간결한 설명
  - 수치 직접 나열 지양, 자연스러운 설명
  - 제공된 데이터만을 근거로 한 객관적 분석
...
```json
{
  "analysis": "생성된 분석 텍스트 (최대 2줄)"
}
```

**에러 처리**:
- API 키 미설정 시 명확한 에러 메시지
- OpenAI API 호출 실패 시 상세 에러 정보
- 유효하지 않은 분석 타입 검증
